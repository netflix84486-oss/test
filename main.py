...
import nodriver as uc  # pip install nodriver fastapi uvicorn

app = FastAPI(title="RTO Automation API", version="1.0.0")

# Windows: asyncio subprocess policy
if os.name == "nt":
    try:
...
async def run_flow(
    ...
    async def main():
        ...
        # NOTE: On Render we must run headless
...
def run_in_child_sync(reg_no: str, chassis_no: str, rto_value: str, headless: bool, timeout_sec: int) -> Dict[str, Any]:
    fd, out_path = tempfile.mkstemp(prefix="rto_child", suffix=".json")
    os.close(fd)
    ctx = get_context("spawn")
    p = ctx.Process(target=_child_run_flow, args=(reg_no, chassis_no, rto_value, headless, timeout_sec, out_path), daemon=True)
    p.start()
    p.join(timeout_sec + 60)
    if p.is_alive():
        try: p.terminate()
...
@app.get("/")
def health():
    return {"ok": True, "service": "RTO Automation API"}


@app.get("/search")
async def search(regno: str, chassis: str, rto_value: str = "53", timeout_sec: int = 120):
    """Public GET endpoint: /search?regno=...&chassis=..."""
    # Always headless on server
    out = await asyncio.to_thread(run_in_child_sync, regno, chassis, rto_value, True, timeout_sec)
...
async def run_endpoint(
    ...
    out = await asyncio.to_thread(run_in_child_sync, reg_no, chassis_no, rto_value, headless, timeout_sec)
    return JSONResponse(content=out)
